...
function rebuildChevrons(highlightedTasks, overdueToday) {
    ...
    for (var i = 0; i < taskRowInfoBlocks.length; i++) {
        ...
        if (section.id == "completedContent" && taskTitle.endsWith("]")) {
            ...
            completedActualMinutes += minutes;
        }
    }

    // console.log("RESULT: " + (todayMinutes / 60) + " -- "
    // + (weekMinutes / 60) + " -- " + (laterMinutes / 60));
    updateHours("todayHeader", `запланировано: ${prettyMinutes(todayMinutes)}`);
    updateHours("weekHeader", `запланировано: ${prettyMinutes(weekMinutes)}`);
    updateHours("laterHeader", `запланировано: ${prettyMinutes(laterMinutes)}`);
    updateHours("completedHeader",
      `планово: ${prettyMinutes(completedPlannedMinutes)} -- фактически: ${prettyMinutes(completedActualMinutes)}`);
}
